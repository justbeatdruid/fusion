...
local function log(message)
  ...
  if request_latency and request_latency >= 0 then
    metrics.latency:observe(request_latency, { "request", service_name, route_name, consumer_id })
  end

  local upstream_latency = message.latencies.proxy
  if upstream_latency ~= nil and upstream_latency >= 0 then
    metrics.latency:observe(upstream_latency, { "upstream", service_name, route_name, consumer_id })
  end

  local kong_proxy_latency = message.latencies.kong
  if kong_proxy_latency ~= nil and kong_proxy_latency >= 0 then
    metrics.latency:observe(kong_proxy_latency, { "kong", service_name, route_name, consumer_id })
  end
end
