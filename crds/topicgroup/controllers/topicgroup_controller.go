...
func (r *TopicgroupReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if namespace.Status.Status == nlptv1.Creating || namespace.Status.Status == nlptv1.Importing {
		...
		if err := r.Operator.CreateTenantIfNotExist(namespace.ObjectMeta.Namespace); err != nil {
			...
			if namespace.Status.Status == nlptv1.Importing {
				namespace.Status.Status = nlptv1.ImportFailed
			}
		...
		if err := r.Operator.CreateNamespace(namespace); err != nil {
			...
			if namespace.Status.Status == nlptv1.Importing {
				namespace.Status.Status = nlptv1.ImportSuccess
			}
		...
		if err != nil || ns == nil {
			...
			if dstPolicies != nil {
				...
				if err = r.Operator.SetPolicy(subscriptionDispatchRateSuffix, namespace, namespace.Spec.Policies.SubscriptionDispatchRate, post); err != nil {
					namespace.Status.Status = nlptv1.UpdateFailed
					namespace.Status.Message = fmt.Sprintf("set subscriptionDispatchRate: %+v", err)
				}

				//更新TopicDispatchRate
				if err = r.Operator.SetPolicy(dispatchRateSuffix, namespace, namespace.Spec.Policies.TopicDispatchRate, post); err != nil {
					namespace.Status.Status = nlptv1.UpdateFailed
					namespace.Status.Message = fmt.Sprintf("set dispatchRate: %+v", err)
				}
