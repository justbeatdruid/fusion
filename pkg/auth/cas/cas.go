...
func (*cas) GetUserByID(id string) (User, error) {
	...
	if responseBody.Code != 200 {
		return User{}, fmt.Errorf("request for getting cas user error: received cod is not 200: desc: %s", responseBody.Desc)
	}
...
func (*cas) ListUsers() ([]User, error) {
	...
	if responseBody.Code != 200 {
		return nil, fmt.Errorf("request for getting cas user error: received cod is not 200: desc: %s", responseBody.Desc)
	}
